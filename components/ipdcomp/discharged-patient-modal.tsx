...
export default function DischargedPatientsModal({ isOpen, onClose, patients }: DischargedPatientsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-transparent bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div
        ref={modalRef}
        className="bg-white rounded-lg shadow-xl w-full max-w-[95%] min-w-[80%] max-h-[90vh] overflow-hidden flex flex-col"
      >
        {/* Header */}
        <div className="bg-blue-500 text-white px-6 py-4 flex items-center justify-between">
          ...
          <Button
            ...
            size="icon"
            onClick={onClose}
            className="text-white hover:bg-blue-600 rounded-full cursor-pointer h-8 w-8"
          >
            <X className="h-5 w-5" />
          </Button>
        </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <div className="flex space-x-2">
            <Button variant="outline" className="flex cursor-pointer hover:bg-gray-200/40 items-center space-x-2">
        ...
        <div className="flex-1 overflow-auto">
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50 divide-x divide-gray-200 border-b border-gray-200">
            ...
            <TableBody>
              {filteredPatients.map((patient) => (
                <TableRow key={patient.id} className="hover:bg-gray-50 divide-x divide-gray-200 border-b border-gray-200">
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8 cursor-pointer p-2 hover:bg-blue-200/40">
                        <Eye className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 cursor-pointer p-2 hover:bg-gray-200/40">
                        <Download className="w-4 h-4" />
                      </Button>
                    </div>
                  </TableCell>
